...
export default function TxList({ items }) {
  if (!items || !items.length) {
    return (
      <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-800 bg-slate-950/50 py-8 text-center">
        <Activity className="h-6 w-6 text-slate-500 mb-1" />
        <p className="text-xs font-semibold text-slate-300">No session transactions</p>
        <p className="mt-0.5 text-[11px] text-slate-500">Sign a transaction to view execution state here.</p>
      </div>
    )
  }
    ...
    <ul className="space-y-2.5">
      {items.map((tx) => (
        <li
          ...
          className="flex items-center justify-between rounded-xl border border-slate-800 bg-slate-950 p-3 shadow-sm transition hover:border-slate-700"
        >
          ...
          <span
            className={`inline-flex shrink-0 items-center gap-1 rounded-full border px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider ${
              tx.status === 'confirmed'
                ...
                : 'border-amber-800 bg-amber-950 text-amber-300 animate-pulse'
            }`}
          >
